...
- Загрузка секций идёт через экспортированный клиентский API **`Lampa.Api.sources.tmdb.list`**. Не использовать напрямую `Lampa.TMDB`: в bundled Samsung/Lampa runtime он не экспортируется наружу, и меню тогда падает обратно в плоский список без секций.
- Каждая секция сохраняет Discover URL, текущую страницу и признак следующей страницы для кнопки **«Ещё»**. Если Lampa открывает следующую страницу, а TMDB/прокси вернул пустой ответ, источник повторно открывает первую страницу секции вместо пустого экрана.

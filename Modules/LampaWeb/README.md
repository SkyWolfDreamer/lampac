...
- Загрузка секций идёт напрямую через **`Lampa.Reguest`** к TMDB Discover TV и использует такой же URL-builder, как встроенный TMDB source: **`Lampa.Utils.protocol()`**, настройку **`proxy_tmdb`**, `api_key`, `language` и нужный `page`. Не использовать напрямую `Lampa.TMDB`: в bundled Samsung/Lampa runtime он не экспортируется наружу, и меню тогда падает обратно в плоский список без секций.

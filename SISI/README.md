...
Плагин **`/sisi.js`** также добавляет не-adult пункт **«Дорамы»** в основное меню Lampa сразу после **«Сериалы»**. Это нужно для внешнего сценария подключения Lampac через **`/on.js`**, где нет доступа к bundled-коду меню Lampa; кнопка регистрирует источник **`lampac_dorama`** и открывает экран с секциями **«Сейчас смотрят»**, **«Новые серии»**, **«Онгоинги»**, **«Популярное»**, **«Последнее добавление»**, **«Новинки этого года»**, **«С высоким рейтингом»**. Все секции используют прямой TMDB Discover TV фильтр корейских драм через **`Lampa.Reguest`** и URL-builder по нативному паттерну TMDB source (**`Lampa.Utils.protocol()`** + **`proxy_tmdb`** + page); **«Онгоинги»** требуют серии с `air_date` от сегодня до ближайших 21 дней, а не просто TMDB-статус `Returning Series`; новые серии и новые тайтлы не показывают будущие релизы. Секционные строки сохраняют исходный Discover URL и page metadata для кнопки **«Ещё»** и открывают следующую страницу по тому же паттерну, что **«Сериалы»**: без отката на первую страницу. Не использовать напрямую `Lampa.TMDB`: в реальном клиенте он не экспортируется, и тогда открывается fallback-плоский список без секций. Если другой init-путь уже добавил кнопку ниже по меню, SISI переносит её после **«Сериалы»**, чистит дубли и перерегистрирует устаревший `lampac_dorama`.

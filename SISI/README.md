...
Плагин **`/sisi.js`** также добавляет не-adult пункт **«Дорамы»** в основное меню Lampa сразу после **«Сериалы»**. Это нужно для внешнего сценария подключения Lampac через **`/on.js`**, где нет доступа к bundled-коду меню Lampa; кнопка регистрирует источник **`lampac_dorama`** и открывает экран с секциями **«Сейчас смотрят»**, **«Новые серии»**, **«Онгоинги»**, **«Популярное»**, **«Последнее добавление»**, **«Новинки этого года»**, **«С высоким рейтингом»**. Все секции используют прямой TMDB Discover TV фильтр корейских драм через экспортированный **`Lampa.Api.sources.tmdb.list`**. Не использовать напрямую `Lampa.TMDB`: в реальном клиенте он не экспортируется, и тогда открывается fallback-плоский список без секций. Если другой init-путь уже добавил кнопку ниже по меню, SISI переносит её после **«Сериалы»** и чистит дубли.

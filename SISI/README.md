...
Клиентская настройка **«Отображать в меню»** в плагине SISI применяется сразу: пункт **«Клубничка»** добавляется или удаляется из главного меню Lampa без перезапуска клиента/сервера.

Плагин **`/sisi.js`** также добавляет не-adult пункт **«Дорамы»** в основное меню Lampa сразу после **«Сериалы»**. Это нужно для внешнего сценария подключения Lampac через **`/on.js`**, где нет доступа к bundled-коду меню Lampa; кнопка регистрирует источник **`lampac_dorama`** и открывает экран с секциями **«Сейчас смотрят»**, **«Новые серии»**, **«Онгоинги»**, **«Популярное»**, **«Последнее добавление»**, **«Новинки этого года»**, **«С высоким рейтингом»**, **«Комедийные дорамы»**, **«Криминальные»**, **«Детективы»**, **«Боевики»**, **«Фэнтези»**, **«Семейные»**, **«Мини-сериалы»**, **«Netflix»**, **«tvN»**, **«JTBC»**. Все секции используют TMDB Discover TV фильтр корейских драм через **`Lampa.Reguest`**, но запросы идут в штатный Lampac TMDB proxy **`/tmdb/api/3/...`**; это снимает зависимость от прямого CORS-доступа к TMDB и внешнего `apitmdb.cub.watch`. Если **`{localhost}`** не заменён, остаётся fallback на нативный TMDB URL-builder (**`Lampa.Utils.protocol()`** + **`proxy_tmdb`** + page). **«Онгоинги»** требуют серии с `air_date` от сегодня до ближайших 21 дней, а не просто TMDB-статус `Returning Series`; новые серии и новые тайтлы не показывают будущие релизы. Секционные строки сохраняют исходный Discover URL и page metadata для кнопки **«Ещё»** и открывают следующую страницу по тому же паттерну, что **«Сериалы»**: без отката на первую страницу. Жанровые строки используют проверенные TMDB TV filters: `18,35`, `18,80`, `18,9648`, `18,10759`, `18,10765`, `18,10751`, тип `with_type=2`, а также networks `213` (Netflix), `866` (tvN), `885` (JTBC). Init-код отдельно перехватывает Dorama `category_full` URL, потому что нативная кнопка **«Ещё»** при active source CUB может передать `source=cub`; такие ссылки принудительно маршрутизируются в `lampac_dorama`, а старые открытые `source=cub` / `source=tmdb` ссылки перехватываются в `list`. Не использовать напрямую `Lampa.TMDB`: в реальном клиенте он не экспортируется, и тогда открывается fallback-плоский список без секций. Если другой init-путь уже добавил кнопку ниже по меню, SISI переносит её после **«Сериалы»**, чистит дубли и перерегистрирует устаревший `lampac_dorama`.

Иконка пункта **«Дорамы»** задаётся inline SVG в Hangul-style и принудительно обновляется даже для уже созданной кнопки, чтобы старый placeholder или иконка TV не оставались в DOM после hot reload / повторного init.
